...
public class TestSuiteGeneratorHelper {
	...
	public static void writeEventableTestDataToJson(String fname, Collection<Eventable> eventables) throws IOException {
		...
		for (Eventable eventable : eventables) {
			Eventable newEventable = new Eventable(eventable.getIdentification(), eventable.getEventType(),
	...
	public List<TestMethod> getTestMethods() {
		...
		return getTestMethods(crawlPaths, null);
	...
	public static String getEventableInfo(Eventable eventable) {
		if ((eventable.getEdgeSource() instanceof LinkedTreeMap)
			...
			return MoreObjects.toStringHelper(eventable).add("eventType", eventable.getEventType())
					.add("identification", eventable.getIdentification()).add("element", eventable.getElement())
					.add("source",
							"" + (int) Double.parseDouble(
									((LinkedTreeMap<?, ?>) eventable.getEdgeSource()).get("id").toString()))
					.add("target", "" + (int) Double
	...
	public static List<TestMethod> getTestMethods(Collection<List<Eventable>> crawlPaths,
			List<MethodLabelObject> labelMap) {
		...
		for (List<Eventable> crawlPath : crawlPaths) {
			...
			/* Get method labels */
			MethodLabelObject methodLabels = null;
			if (labelMap != null) {
				try {
					methodLabels = labelMap.get(iter);
				} catch (Exception ex) {
					ex.printStackTrace();
				}
			}
			...
			for (Eventable clickable : crawlPath) {
				...
				/* Label for clickable */
				try {
					String clickableLabel = methodLabels.getLabel_map().get(new Integer((int) clickable.getId()))
							.getLabel();
					properties.put("label", clickableLabel);
				} catch (Exception ex) {
					ex.printStackTrace();
				}
				...
				if (clickable.getElement() != null)
					properties.put("text", clickable.getElement().getText().replaceAll("\"", "\\\\\"").trim());
				else
					properties.put("text", "");
				try {
					...
					if (clickable.getEdgeSource() instanceof LinkedTreeMap) {
						sourceid = "" + (int) Double
								.parseDouble(((LinkedTreeMap<?, ?>) clickable.getEdgeSource()).get("id").toString());
					}
				...
				try {
					...
					if (clickable.getEdgeTarget() instanceof LinkedTreeMap) {
						targetid = "" + (int) Double
								.parseDouble(((LinkedTreeMap<?, ?>) clickable.getEdgeTarget()).get("id").toString());
					}
