...
if [[ $PRODUCTION == "yes"  ]]; then
	...
	if [[ $CONTAINER_NAME != "default" ]]; then
		docker run -idt --workdir=/var/www/html/pagekit --name=$CONTAINER_NAME --expose 80 --expose 3306 -p $PORT_APP:80 -p $PORT_DB:3306 dockercontainervm/pagekit:latest bash
	else
		docker run -idt --workdir=/var/www/html/pagekit --expose 80 --expose 3306 -p $PORT_APP:80 -p $PORT_DB:3306 dockercontainervm/pagekit:latest bash
	fi
fi
