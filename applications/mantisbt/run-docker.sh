...
if [[ $PRODUCTION == "yes"  ]]; then
	...
	if [[ $CONTAINER_NAME != "default" ]]; then
		docker run -idt --workdir=/home --name=$CONTAINER_NAME --expose 80 --expose 3306 -p $PORT_APP:80 -p $PORT_DB:3306 dockercontainervm/mantisbt:1.2.0 bash
	else
		docker run -idt --workdir=/home --expose 80 --expose 3306 -p $PORT_APP:80 -p $PORT_DB:3306 dockercontainervm/mantisbt:1.2.0 bash
	fi
fi
