...
if [[ $PRODUCTION == "yes"  ]]; then
	...
	if [[ $CONTAINER_NAME != "default" ]]; then
		docker run -idt --workdir=/home/splittypie --name=$CONTAINER_NAME --expose 4200 -p $PORT_APP:4200 dockercontainervm/splittypie:latest bash
	else
		docker run -idt --workdir=/home/splittypie --expose 4200 -p $PORT_APP:4200 dockercontainervm/splittypie:latest bash		
	fi
fi
