...
if [[ $PRODUCTION == "yes"  ]]; then
	...
	if [[ $CONTAINER_NAME != "default" ]]; then
		docker run -idt --workdir=/home/phoenix-trello --name=$CONTAINER_NAME --expose 4000 --expose 5432 -p $PORT_APP:4000 -p $PORT_DB:5432 dockercontainervm/phoenix-trello:latest bash 
	else
		docker run -idt --workdir=/home/phoenix-trello --expose 4000 --expose 5432 -p $PORT_APP:4000 -p $PORT_DB:5432 dockercontainervm/phoenix-trello:latest bash
	fi
fi
