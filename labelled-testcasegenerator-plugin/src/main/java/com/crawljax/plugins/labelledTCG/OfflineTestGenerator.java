package com.crawljax.plugins.labelledTCG;
...
import com.crawljax.plugins.labelledTCG.crawlPlugins.AddressbookCleanup;
import com.crawljax.plugins.labelledTCG.crawlPlugins.ClarolineCleanup;
import com.crawljax.plugins.labelledTCG.util.GsonUtils;
...
public class OfflineTestGenerator {
	...
	public static final String JSON_STATES = LabelledTestSuiteGenerator.TEST_SUITE_PATH + "states.json";
	public static final String JSON_EVENTABLES = LabelledTestSuiteGenerator.TEST_SUITE_PATH + "eventables.json";
	...
	public static boolean verifyCrawlPathInfo(List<Eventable> path, CrawlPathInfo pathInfo) {
		...
		try {
			...
			String printPath = printCrawlPath(path, false).substring(printCrawlPath(path, false).indexOf("_"));
	...
	public static String generateTestCases(Collection<List<Eventable>> crawlPaths, OutPutModel outputModel,
			JsonObject configObject, String absPath, String labelPath) {
		...
		List<MethodLabelObject> labelMap = UtilsLabelling.importLabelJson(labelPath);

		List<TestMethod> testMethods = TestSuiteGeneratorHelper.getTestMethods(crawlPaths, labelMap);
		...
		try {
			JavaTestGenerator generator = new JavaTestGenerator(LabelledTestSuiteGenerator.CLASS_NAME, initialURL, testMethods,
					config, // Config,
					absPath,
			...
			String generatedFileName = generator.generate(
					DomUtils.addFolderSlashIfNeeded(absPath + LabelledTestSuiteGenerator.TEST_SUITE_PATH),
					LabelledTestSuiteGenerator.FILE_NAME_TEMPLATE);
			String generatedFileName2 = generator.generate(
					DomUtils.addFolderSlashIfNeeded(absPath + LabelledTestSuiteGenerator.TEST_SUITE_PATH_LABELS),
					LabelledTestSuiteGenerator.FILE_NAME_TEMPLATE_LABELS);
			if (null != generatedFileName) {
				generator.copyExecutionScripts(absPath, LabelledTestSuiteGenerator.TEST_SUITE_SRC_FOLDER,
						LabelledTestSuiteGenerator.TEST_SUITE_PACKAGE_NAME, LabelledTestSuiteGenerator.CLASS_NAME);
			}
			LOGGER.info("Generated : " + generatedFileName);
			LOGGER.info("Generated : " + generatedFileName2);
		} catch (Exception e) {
	...
	public static void main(String args[]) {
		...
		if (args.length != 2) {
			LOG.info("Usage: "
					+ "Argument 1: absolute location of the crawl folder."
					+ "Argument 2: absolute location of label json");
		}
		String crawlLocation = args[0];
		String labelLocation = args[1];
		...
		if (crawlPathsJson.exists()) {
			...
			try {
				 ...
				 */
				generateTestCases(crawlPaths, result, configObject, crawlLocation, labelLocation);
